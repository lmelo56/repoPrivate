...
class Ai1wmge_Settings {
	...
	public function revoke() {
		...
		// Remove if extension is using app folder access
		delete_option( 'ai1wmge_gdrive_app_folder' );
	...
	public function get_next_backup_date( $schedules ) {
		...
		foreach ( $schedules as $schedule ) {
			$future_backup_timestamps[] = wp_next_scheduled( "ai1wmge_gdrive_{$schedule}_export", array( $this->get_cron_args() ) );
		}
	...
	public function set_cron( $schedules ) {
		...
		foreach ( $schedules as $schedule ) {
			Ai1wm_Cron::add( "ai1wmge_gdrive_{$schedule}_export", $schedule, $this->get_cron_timestamp(), array( $this->get_cron_args() ) );
		}
	...
	public function init_cron() {
		foreach ( $this->get_cron() as $schedule ) {
			if ( ! Ai1wm_Cron::exists( "ai1wmge_gdrive_{$schedule}_export", array( $this->get_cron_args() ) ) ) {
				Ai1wm_Cron::clear( "ai1wmge_gdrive_{$schedule}_export" );
				Ai1wm_Cron::add( "ai1wmge_gdrive_{$schedule}_export", $schedule, $this->get_cron_timestamp(), array( $this->get_cron_args() ) );
			}
		}
	}
	...
	public function get_cron_args() {
		if ( $this->get_incremental() ) {
			return array( 'secret_key' => get_option( AI1WM_SECRET_KEY ), 'incremental' => 1, 'gdrive' => 1 );
		}

		return array( 'secret_key' => get_option( AI1WM_SECRET_KEY ), 'gdrive' => 1 );
	}
	...
	public function set_incremental_folder_id( $folder_id ) {
		return update_option( 'ai1wmge_gdrive_incremental_folder_id', $folder_id );
	}

	public function get_incremental_folder_id() {
		return get_option( 'ai1wmge_gdrive_incremental_folder_id', false );
	}
	...
	public function set_incremental( $incremental ) {
		return update_option( 'ai1wmge_gdrive_incremental', $incremental );
	}

	public function get_incremental() {
		return get_option( 'ai1wmge_gdrive_incremental', false );
	}

	public function set_app_folder( $value ) {
		return update_option( 'ai1wmge_gdrive_app_folder', $value );
	}

	public function get_app_folder() {
		return get_option( 'ai1wmge_gdrive_app_folder', null );
	}
}
