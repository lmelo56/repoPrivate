...
class Ai1wmge_Export_Upload {
	...
	public static function execute( $params, Ai1wmge_GDrive_Client $gdrive = null ) {
		...
		if ( ! isset( $params['archive_size'] ) ) {
			throw new Ai1wm_Import_Exception( __( 'Google Drive Archive Size is not specified.', AI1WMGE_PLUGIN_NAME ) );
		}
		...
		if ( ( fseek( $archive, $params['archive_offset'] ) !== -1 )
			...
			try {
				...
				if ( ( $response = $gdrive->upload_file_chunk( $file_chunk_data, $params['archive_size'], $params['file_range_start'], $params['file_range_end'] ) ) ) {
				...
				if ( $params['upload_retries'] <= apply_filters( 'ai1wmge_default_api_retries', AI1WMGE_DEFAULT_API_RETRIES ) ) {
					if ( isset( $params['upload_backoff'] ) ) {
